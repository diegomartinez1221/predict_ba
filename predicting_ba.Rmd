...
I begin by building a random forest model with every predictor in the dataset. I will then check the variable importance to tune the model. 
```{r echo=TRUE}
set.seed(123)
sample <- sample.int(n = nrow(batting), size = floor(.8*nrow(batting)), replace = FALSE)
...
trainer<-trainer%>% select(- playerid, -Team, -MarApr_HR, -MarApr_SB, - MarApr_HR.FB, -MarApr_R, -MarApr_RBI, -MarApr_BB.)
...
predict_ba<- predict(fit_rf, tester)

tester%>%mutate(predicted_fullseason_avg = predict(fit_rf, .))%>%
  mutate(se = (FullSeason_AVG - predicted_fullseason_avg)^2)%>%
