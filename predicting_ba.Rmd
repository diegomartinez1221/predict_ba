...
lifetime_avg<-cumulative_stats_2017 %> %mutate(Lifetime_AVG = AVG) %>% select(Name, Lifetime_AVG)
...
```{r fig.align='center'}
batting%>%
  ggplot(aes(x = MarApr_AVG, y = FullSeason_AVG)) + geom_point() +
  ylim(0, 0.5) +
  xlim(0,0.5)+ 
...
```{r echo=TRUE}
set.seed(1234)
sample <- sample.int(n = nrow(batting), size = floor(.7*nrow(batting)), replace = FALSE)
...
prediction = data.frame(Name = tester$Name, FullSeason_AVG = tester$FullSeason_AVG, prediction = predict(fit_rf, newdata = tester), MarApr_AVG = tester$MarApr_AVG)
...
prediction%>%
  ...
  ggplot(aes(x = results, y = ba, group = Name)) + geom_point(alpha = 0.4) + geom_line(alpha = 0.4) + scale_y_continuous(breaks = seq(0.1,0.4,0.05), limits = c(0.1,0.4))  + geom_point(alpha = 0.4) + labs(title = "Random Forest Model Predicted Regression to the Mean")

prediction%>%
  gather(FullSeason_AVG, prediction, key = results, value = ba)%>%
  ggplot(aes(x = results, y = ba, group = Name)) + geom_point(alpha = 0.4) + geom_line(alpha = 0.4) + scale_y_continuous(breaks = seq(0.1,0.4,0.05), limits = c(0.1,0.4)) + labs(title = "Model Did Not Predict a  Wide Range of Batting Averages", subtitle = ) + geom_hline(yintercept = mean(prediction$Fullseason_AVG, na.rm = TRUE), linetype = 3, color = "red") + geom_hline(yintercept = mean(prediction$prediction, na.rm = TRUE), linetype = 3)
...
```{r}
prediction<-prediction%>%mutate(se = (FullSeason_AVG - prediction)^2, difference = FullSeason_AVG - prediction)
...
```{r}
prediction%>%
  arrange(desc(abs(difference)))%>% head()

prediction%>%
  arrange(abs(difference)) %>% head ()

hist(prediction$difference)
